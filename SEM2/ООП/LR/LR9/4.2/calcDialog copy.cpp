...
#define CLR_ALL 32
 ...
 // количество кнопок в группе, отображаемой в виде сетки
#define GRID_KEYS 16

/// Описатель кнопки
struct BtnDescr{
    QString text; ///< Отображаемый на кнопке текст
...
/// Динамический массив-вектор элементов описателей кнопок
...
/// MY CODE !!!!!!!!!!!!!!!!!
_btnDescr.push_back( BtnDescr("pow", POW) );
...
CalcDialog::CalcDialog( QWidget * parent) {
    ...
    // устанавливаем режим только чтения - разрешаем ввод только // с нарисованных кнопок
    ...
    /// MY CODE !!!
    QVBoxLayout *funcBtnLayout = new QVBoxLayout();
    ...
    // обеспечиваем соответствие кнопки её идентификатору
    ...
    /// MY CODE !!!
        ...
        /// MY CODE !!!!!!!!!!!!!!!!!
        funcBtnLayout->addWidget(button);
    else
     {
        mainKeysLayout->addLayout(funcBtnLayout); /// MY CODE !!!!!!!!!!!!!!!!!
    ...
    // нашего класса
    connect(m_pSignalMapper, SIGNAL(mapped(int)), this, SLOT(clicked(int)));

    // добавляем блоки кнопок в схему выравнивания всей формы
    dlgLayout->addWidget(m_pLineEdit);
    dlgLayout->addLayout(bccKeysLayout);
    dlgLayout->addLayout(mainKeysLayout);

    // связываем схему выравнивания dlgLayout с формой
      setLayout(dlgLayout);
...
void CalcDialog::clicked(int id) { // по идентификатору кнопки ищем действие для выполнения
  switch(id){
...
case DOT:     // добавление десятичной точки
{
// если на экране результат предыдущей операции - сбросить
    checkOpPerf();
...
case CLR_ALL: initNum(); // удалить всё
case CLR:{
    setNumEdit( 0 ); // записать в m_pLineEdit число 0
    break;
}

case BKSP:{ // удалить последний символ
// если на экране резульатат предыдущей операции - сбросить
checkOpPerf();
QString str = m_pLineEdit->text ();
if( str.length() ){
...
QString sId;
 ...
 // сформировать строку по идентификатору нажатой клавиши
  sId.setNum( id );
  QString str =  m_pLineEdit->text ();
  if( str == "0" )
      str = sId; // затираем незначащий нуль
  else
...
m_Op = curOp; // запомнить результат текущей операции
